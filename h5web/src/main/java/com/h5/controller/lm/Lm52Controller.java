...
@Controller
@RequestMapping("lm52")
public class Lm52Controller {

    @Autowired
    private Lm52Service lm52Service;
    @RequestMapping("/list")
    public ModelAndView list(Integer pageNum, Integer pageSize) throws Exception {
        ...
        ModelAndView modelAndView = new ModelAndView("lm/lm52List");
        PageHelper.startPage(pageNum, pageSize);//分页
        Map<String, Object> dataMap = new HashMap<String, Object>();
        List<LM52> list = lm52Service.list();//开始查询
        PageInfo<LM52> pageInfo = new PageInfo<>(list);//封装page
        modelAndView.addObject("pageInfo", pageInfo);
        return modelAndView;
    }
    @RequestMapping("/addOrUpdate")
    public ModelAndView addOrUpdate(Long id) throws Exception {
        ModelAndView modelAndView = new ModelAndView("/lm/lm52Edit");

        if (null != id && !"".equals(id)) {
            //编辑
            LM52 lm52 = lm52Service.getById(id);
            modelAndView.addObject("lm", lm52);
        }
        //跳转到新增页面
        return modelAndView;
    }
    ...
    @ResponseBody
    public RespFormat saveOrUpdate(LM52 lm52) throws Exception {
//        String name = menu.getName();
        ...
        try {
            ...
            if(null != id && !"".equals(id)){
                //修改
               i=lm52Service.update(lm52);
            }else{
               i=lm52Service.save(lm52);
            }
        }catch (Exception e){
